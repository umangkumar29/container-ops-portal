  ...
  if (AUTH_BYPASS) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0B0F19] text-white font-sans">
        <div className="max-w-lg p-10 rounded-3xl bg-white/[0.04] border border-white/[0.08]">
          ...
          <h1 className="text-3xl font-semibold text-white mb-4">Authentication bypassed</h1>
          <p className="text-slate-300 leading-relaxed mb-6">
            Frontend authentication is disabled for CLI-driven workflows. Make sure you have run
            <code className="mx-1 px-2 py-1 text-sm bg-white/[0.08] rounded-md">az login</code>
            in your terminal before using the portal so the backend can reuse your Azure session.
          </p>
          <button
            onClick={() => window.location.replace("/")}
            className="w-full rounded-xl bg-[#6366F1] hover:bg-[#4F46E5] transition-colors px-6 py-3 font-semibold"
          >
            Enter portal
          </button>
        </div>
